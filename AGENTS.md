...
Five of six additions from one batch (all from a menu of enhancement
ideas the user picked from directly — the sixth, Open Graph image, was
built and then explicitly reverted, see below):
...
- **Person structured data (JSON-LD)** — `BaseLayout.astro` builds a
  ...
  `sameAs` pulls whichever of `linkedin`/`github`/`twitter` are set. No
  `image` field — that would have pointed at the OG image, which was
  reverted (below); left unset rather than pointing at something that
  no longer exists.
- **Open Graph image — built, then explicitly reverted, do not
  re-add without being asked.** Originally: `public/og-image.png`
  (1200×630) generated by a `scripts/generate-og-image.mjs` script
  (`npm run generate:og`) that hand-built an SVG matching the site's
  palette/Seal motif and rasterized it with `sharp`, plus
  `og:image`/`twitter:image` meta tags and `twitter:card` set to
  `summary_large_image` in `BaseLayout.astro`. The user asked for this
  reverted — the script, the generated PNG, the `generate:og` npm
  script, and all of the associated meta tags are gone;
  `twitter:card` is back to plain `summary`, matching how it was
  before this feature existed. **Sharing a link currently shows no
  preview image again** — that's the deliberate result of the
  revert, not a bug.
