// Shared logic between the scheduled notify-subscribers.mts and the
// manual test-trigger notify-subscribers-test.mts — same run, two
// different ways to invoke it. See notify-subscribers.mts for the full
...
export async function runNotify(): Promise<string> {
  const siteUrl = process.env.URL ?? process.env.DEPLOY_URL;
  const resendApiKey = process.env.RESEND_API_KEY;
  const fromEmail = process.env.RESEND_FROM_EMAIL ?? "onboarding@resend.dev";

  if (!siteUrl || !resendApiKey) {
    return "Missing configuration: URL or RESEND_API_KEY is not set.";
  }
  ...
  if (newPosts.length === 0) {
    return `No new posts since last run (last run cutoff: ${lastNotifiedAt}).`;
  }
  ...
  if (blobs.length === 0) {
    return `Found ${newPosts.length} new post(s), but there are 0 subscribers in the store.`;
  }
  ...
  const summary = `Notified ${emailsSent}/${blobs.length} subscriber(s) about ${newPosts.length} new post(s).`;
